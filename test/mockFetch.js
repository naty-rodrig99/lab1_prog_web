...
function checkFetchUrl(url, param, [hashCode1, hashCode2], queryHash=[]){
    ...
    expect(Object.keys(param.headers).length, "exactly two headers needed in the lab: the API key and the group number. Check the slides for the right header name and value, they are different from the API documentation.").to.equal(2);
    ...
    expect(Object.keys(param.headers).find(x=> hash(x.toLowerCase())===158538605 && hash(param.headers[x].toString())===hash(group)), "the group in the header should match the group in the url").to.be.ok;
